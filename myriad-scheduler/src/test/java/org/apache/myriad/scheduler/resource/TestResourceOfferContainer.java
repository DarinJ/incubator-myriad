...
public class TestResourceOfferContainer {
  ...
  private List<ServiceResourceProfile> getProfiles(MyriadConfiguration conf) {
    ...
    List<ServiceResourceProfile> profileList = new ArrayList<>();
    Map<String, Map<String, String>> profiles = cfg.getProfiles();
    for (Map.Entry<String, Map<String, String>> entry : profiles.entrySet()) {
      Map<String, String> resourceMap = entry.getValue();
      Long cpu = Long.parseLong(resourceMap.get("cpu"));
      Long mem = Long.parseLong(resourceMap.get("mem"));
      profileList.add(new ExtendedResourceProfile(new NMProfile(entry.getKey(), cpu, mem), taskUtils.getNodeManagerCpus(),
