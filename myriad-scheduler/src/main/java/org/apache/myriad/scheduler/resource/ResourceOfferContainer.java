...
public class ResourceOfferContainer {
  ...
  private RangeResource ports;
  private Boolean meetCriteria;
   ...
   */
  public ResourceOfferContainer(Protos.Offer offer, ServiceResourceProfile profile) {
    this.offer = offer;
    setScalarValues();
    ports = new RangeResource(offer, RESOURCE_PORTS, profile.getPorts().values());
  }
  ...
  public List<Protos.Resource> consumePorts(Collection<Long> requestedValues) {
    return ports.consumeResource(requestedValues);
  }
  ...
  private void addToScalarResource(String name, Double value, Boolean hasRole) {
    if (scalarValues.containsKey(name)) {
      ...
      scalarValues.put(name, new ScalarResource(name));
      scalarValues.get(name).incrementValue(value, hasRole);
    }
  }
