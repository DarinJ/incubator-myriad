...
public class ServiceTaskFactoryImpl implements TaskFactory {
  ...
  public TaskInfo createTask(Offer offer, FrameworkID frameworkId, TaskID taskId, NodeTask nodeTask) {
    ...
    CommandInfo commandInfo = clGenerator.generateCommandLine(nodeTask.getProfile(), serviceConfig,
        taskUtils.getPortResources(offer, Lists.newArrayList(portsMap.values()), new HashSet()));
